	...
	: type_specifier 
	  ...
	  compound_statement 
		{	
			...
			//std::cout << "\n";
			$3->generate_code();
			backpatch($3->nextlist,"e");
			genCode("e:");
			if(current_func != "main"){
				genCode(indent+"loadi(ind(ebp), ebp); // restoring dynamic link");
				genCode(indent+"popi(1); //pop stack");
			}
			genCode(indent+"return; //return");
			genCode("}");
		}
	;
...
fun_declarator
	...
	'(' 
	{
		...
		indent = "\t";
		if(current_func != "main"){
			genCode(indent + "pushi(ebp); // Setting dynamic link");
			genCode(indent + "move(esp,ebp); // Setting dynamic link");
		}
	}
	parameter_list 
	...
	{
		$$ = $3;
		STable* temper = globalST.lookup($1);
		temper->offset = -4;
	}
    | IDENTIFIER '(' ')' 
    {
...
statement
        ...
        | IDENTIFIER '(' ')' ';'
	    {
	    	...
	    	if(temp->lookup($1) != NULL){
	    		std::cout << "Error : in line "<< line_no << " : "; 
				std::cout << "Called object which is not a function or function pointer\n";
				ABORT();
	    	}
	    	temp  = globalST.lookup($1);
	    	...
	    	else 
	    	{
	    		if(temp->args.size() != 0){
	    			std::cout << "Error : in line "<< line_no << " : "; 
					std::cout << "Passed less aruguments than expected\n";
					ABORT();
	    		}
		    	$$ = new funstmt_ast($1);
		    	$$->t = temp->returntype;
...
declarator_list
	: declarator 
	{
		...
		if (t->lookup($1->vname)==NULL)
		{
			...
			t->offset -= $1->length;
			for(int i=0;i<$1->length;i=i+4){
				if($<typefunc>0->t->basetype == Type::Int)genCode(indent + "pushi(0); //for "+ ($1->vname)+ " storage");
				else genCode(indent + "pushf(0); //for "+ ($1->vname)+ " storage");
			}
		}
		else 
		{
	...
	| declarator_list ',' 
	{
		...
		if (t->lookup($4->vname)==NULL)
		{
			...
			for(int i=0;i<$4->length;i=i+4){
				if($<typefunc>0->t->basetype == Type::Int)genCode(indent + "pushi(0); //for "+ ($4->vname)+ " storage");
				else genCode(indent + "pushf(0); //for "+ ($4->vname)+ " storage");
			}
		}
		else 
		{
