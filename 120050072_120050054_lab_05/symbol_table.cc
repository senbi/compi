...
Type* result(Type* t1, Type* t2) {
	if (t1->basetype == Type::Int && (t2->basetype == Type::Int || t2->basetype == Type::Float)){
		...
		}else return new Type(Type::Base,Type::Float);
	}
...
dec_ast::dec_ast(dec_ast* t1,int l){
	vname = t1->vname;
	Type* temp = t1->t;
	if(t1->t->tag==Type::Base){
		t1->t = new Type(Type::Array,t1->t);
		t1->t->size = l;
	}
	else{
		for(;temp->pointed->tag != Type::Base;temp = temp->pointed);
		temp->pointed = new Type(Type::Array,temp->pointed);
		temp->pointed->size = l;
	}
	t = t1->t;
	length = l*t1->length;
...
int line_no = 1;
//// Global Variables Corresponding to Code Generation
std::vector<std::string> gCodeArray;
std::string indent;
static int labelno = 0;
std::string newLabel(){
	std::string temp = "label"+ std::to_string(labelno);
	labelno++;
	return temp;
}
void genCode(std::string s){
	gCodeArray.push_back(s);
}
void merge(std::vector<int>&source,std::vector<int>&add){
	for(int i=0;i<add.size();i++){
		source.push_back(add[i]);
	}
}
void backpatch(std::vector<int>&source,std::string label){
	for(int i=0;i<source.size();i++){
		gCodeArray[source[i]-1] += "(" + label +");";
	}
}
